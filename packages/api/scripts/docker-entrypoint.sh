...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  if node -e "
    ...
    const pool = new Pool({ connectionString: process.env.DATABASE_URL || 'postgres://postgres@hailmary-postgres:5432/hailmary' });
...
npm run db:push -w packages/api || {
  echo "⚠️  Database migration failed or schema already up to date"
}
